...
| Smart Skip V1 | Partial | Signed-in server route, durable job leasing, external Batch task storage, worker contract, OpenAI-backed segmenter, UI settings, auto-skip, session-only undo, and pure tests exist. Real Whisper deployment and proactive active-user discovery are follow-ups. |
