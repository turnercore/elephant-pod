...
Physical iPhone/iPad install check:
...
```bash
DAISYPOD_DEVELOPMENT_TEAM=2UTACZ8KMY \
...
- iPhone physical-device build/install passed with team `2UTACZ8KMY`.
- The installed app entitlement used application identifier `2UTACZ8KMY.com.elephanthand.daisypod`, Sign in with Apple, and the `iCloud.com.elephanthand.daisypod` CloudKit container.
- Manual device validation still needs a Sign in with Apple pass in Settings and a protected backend request after sign-in.
