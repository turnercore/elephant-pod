...
export function EpisodeCard({
  ...
  return (
    ...
    >
      <div className="grid min-w-0 grid-cols-[52px_minmax(0,1fr)_auto] items-center gap-2 p-2 md:grid-cols-[64px_minmax(0,1fr)_auto] md:gap-3 md:p-3">
        {artworkUrl ? (
        ...
        <div className="min-w-0">
          ...
          {progress > 0 && !episode.state.played ? <div className="mt-1.5 h-1 overflow-hidden rounded-full border border-bone/15 bg-bone/20"><span className="block h-full bg-coral" style={{ width: `${progress}%` }} /></div> : null}
        </div>
        <div className="flex shrink-0 items-center gap-1">
          <IconButton label={playLabel} active={isCurrentEpisode && isCurrentPlaying} onClick={() => onPlay(episode)} className="h-9 w-9 md:h-10 md:w-10">
            <PlayIcon size={17} fill="currentColor" aria-hidden />
          </IconButton>
          <IconButton label={revealed ? 'Hide episode actions' : 'Show episode actions'} active={revealed} onClick={() => setRevealed((open) => !open)} className="h-9 w-9 md:h-10 md:w-10">
